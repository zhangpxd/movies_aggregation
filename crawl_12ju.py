...
def scrape_sub(slug, cat_id, subcat, max_pages=500, known_ids=None, stop_if_known=False):
    vids = []
    p = 1
    while p <= max_pages:
        ...
        try:
            ...
            if not pv:
                break
            vids.extend(pv)
            if p == 1 and stop_if_known and known_ids is not None and all(v['id'] in known_ids for v in pv):
