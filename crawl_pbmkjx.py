...
用法:
  ...
  python crawl_pbmkjx.py list      # 仅列表+详情(写 data/pbmkjx.json)
  python crawl_pbmkjx.py stream    # 仅抓流(写检查点+streams.json)
  python crawl_pbmkjx.py fold      # 把流合并进 data/pbmkjx.json 并删除冗余 streams.json/cp.json
  python crawl_pbmkjx.py smoke:10  # 冒烟: 取前 10 条验证连通+字段解析, 写 data/pbmkjx_smoke.json(不改生产数据)
"""
...
def main():
    ...
    if mode == 'list':
        ...
        print('用法: python crawl_pbmkjx.py [all|list|stream|fold|smoke:N]')
        sys.exit(1)
